...
Engine::Engine(GLFWwindow *win) : glfwWin(win) {
...
unsigned int Engine::addObject(const char* mesh, const char* name, unsigned int draw_group) {
    return engine.addObject(GameObjectGenData {
        .name = (char*)name,
        .uuid = (char*)"a",
        .mesh_index = engine.render_data.add_mesh(MeshGenData {
            .path = (char*)mesh
        })
    }, draw_group);
}
...
unsigned int Engine::addObject(GameObjectGenData genData, unsigned int draw_group) {
    unsigned int rt = engine.addObject(genData);
    draw_group_manager.groups[draw_group].addObject(rt);
    engine.objects[rt].draw_group = draw_group;
    return rt;
}


unsigned int Engine::addObject(GameObjectGenData genData) {
